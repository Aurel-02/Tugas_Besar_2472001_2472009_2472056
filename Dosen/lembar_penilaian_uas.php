...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($_POST['nilai'] as $nrp => $nilai) {
        ...
        if ($cek->num_rows == 0) {
            $conn->query("
                INSERT INTO tbnilai (id_transkrip, id_mk, nilai_uas, nip)
    ...
    echo "<script>
    alert('Nilai UAS berhasil disimpan');
...
<head>
    <meta charset="UTF-8">
    <title>Lembar Penilaian UAS</title>
            ...
            <?php if ($qMhs->num_rows > 0): ?>
                ...
                <tr>
                    ...
                    <td>
                        <input type="number"
                               name="nilai[<?= $m['nrp'] ?>]"
                               value="<?= htmlspecialchars($m['nilai_uas'] ?? '') ?>"
